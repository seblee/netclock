...
typedef struct _AlarmData //闹钟情報结构体
{
    int32_t AlarmID;                //闹钟唯一识别的ID ELSV是闹钟设定时自动取号
    char AlarmDateTime[25];         //闹钟播报的时间 "yyyy-MM-dd HH:mm:ss"的形式。
    ...
    char AlarmSoundDounloadURL[32]; //从ELSV下载闹钟音的URL
    char AlarmVoiceDounloadURL[32]; //从ELSV下载语音的URL
...
typedef struct _ELAND_DES_S //设备状态结构
{
    ...
    char ElandID[16];                    //Eland唯一识别的ID
    int32_t UserID;                      //用户唯一识别ID，用户登录时获取
    char ElandName[16];                  //Eland名称，用户输入
    int32_t ElandZoneOffset;             //Eland的时区的UTC的offset秒 日本标准时为UTC + 09 : 00  「32400」
    char ElandSerial[20];                //Eland的串口番号。
    char ElandFirmwareVersion[10];       //Eland固件版本号
    char ElandMAC[16];                   //MAC地址
    int32_t ElandDHCPEnable;             //Eland IP地址自动取号是否有效的标志 0 : 無効 1 : 有効
    char ElandIPstr[16];                 //Eland IP地址
    char ElandSubnetMask[16];            //Eland的IPv4子网掩码。
    char ElandDefaultGateway[16];        //Eland的IPv4默认网关。
    int32_t ElandBackLightOffEnable;     //指定時刻内背光是否熄灭的标志  0 : 無効 1 : 有効
    char ElandBackLightOffBeginTime[10]; //设定背光开始熄灭的时刻 "HH:mm:ss"的形式。（ex: "05:00:00"）
    char ElandBackLightOffEndTime[10];   //设定背光开始熄灭的时刻 "HH:mm:ss"的形式。（ex: "05:00:00"）
    char ElandFirmwareUpdateUrl[32];     //固件升级下载的URL
    AlarmData ElandNextAlarmData;        //下次播报予定的闹钟情報
